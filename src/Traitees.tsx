...
export default function Traitees() {
  ...
  // Rappli l'estime avec un curseur sur son site ; ici il est réel — c'est la
  // somme des paniers des chantiers réellement passés au statut "signé".
  const totalSigne = demandes
    .filter((d) => d.statut === 'signe')
    .reduce((somme, d) => somme + (d.panier ?? 0), 0)

  const nbSignes = demandes.filter((d) => d.statut === 'signe').length
  ...
  return (
    <>
      <div className="mb-4 rounded-xl bg-slate-900 px-4 py-4 text-white">
        ...
        <div className="text-3xl font-bold">{euros.format(totalSigne)}</div>
        <div className="text-sm text-slate-400">
          {nbSignes} chantier{nbSignes > 1 ? 's' : ''} — chiffre réel, pas une estimation
        </div>
      </div>
